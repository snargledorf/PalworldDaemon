...
class PalworldServerController(ILogger<PalworldServerController> logger, IConfiguration configuration, IPalworldRCONConnection rconConnection, IDiscordClient discordClient, IPalworldServerExecutable palworldServerExecutable) : IPalworldServerController
{
    ...
    public async Task RunPalworldServerAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            await discordClient.SendMessageAsync("The Palworld server has started!", cancellationToken)
                .ConfigureAwait(false);

            Task serverExitTask = serverProcess!.WaitForExitAsync(cancellationToken);
            ...
            await Task.WhenAny(serverExitTask, delayTask).ConfigureAwait(false);
            ...
            if (!delayTask.IsCompleted)
            {
                ...
                await discordClient.SendMessageAsync("Palworld server crashed", cancellationToken)
                    .ConfigureAwait(false);
                return;
            }

            logger.LogDebug("Sending 5 minute warning to discord");
            await discordClient.SendMessageAsync("Server restarting in 5 minutes", cancellationToken)
                .ConfigureAwait(false);
            logger.LogDebug("Sending 5 minute warning to RCON");
            await rconConnection.BroadcastAsync("Server restarting in 5 minutes", cancellationToken)
                .ConfigureAwait(false);
            ...
            if (palworldServerExecutable.TryGetPalServerProcess(out Process? palworldServerProcess))
                await palworldServerProcess!.WaitForExitAsync(cancellationToken).ConfigureAwait(false);
        }
        else
        {
